...
<div class="page-content container-fluid">
    <div class="footer">
      ...
      <h2 class="" style=" font-weight: 600">Create Password</h2>
        </div>
        <hr style="margin: 0px;">
...
if(isset($_POST['New_pass']))
{
	...
	{
		$query="SELECT * FROM `login` WHERE `password`='$password' AND `user`='admin';";
		$confirm=mysqli_query($conn,$query) or die(mysqli_error());
		if($confirm)
		{
			$query="UPDATE `login` SET `password`='$confirm_pass' WHERE `user`='admin' ";
			$confirm=mysqli_query($conn,$query) or die(mysqli_error());
			if($confirm)
			{
